...
class ExcelWorkspace {
    ...
    # Increment cell movement
    [void] AdvanceX() { $this.pointerX += 1 }
    [void] AdvanceY() { $this.pointerY += 1 }
    
    # Save workbook
    [void] SaveAs( [string]$path ) { $this.workbook.SaveAs($path) }

    # Set value to a cell @x/y coords
    [void] InsertCellNoAdvanceX( [string]$value ) {
        $x = $this.pointerX
        $y = $this.pointerY
        $this.sheet.Cells.Item($x, $y).Value = $value
    }
    # same as above, but auto advances the X coordinate.
    [void] InsertCell( [string]$value ) {
        $this.InsertCellNoAdvance($value)
        $this.AdvanceX()
    } 

    # create header via list of headers
    [void] AddHeader( [string[]]$headers) {
        # Add headers to excel
        $headers | ForEach-Object {
            $this.InsertCell($_)
        }
        # Advance Y so we can add data below
        $this.AdvanceY()
    }
}
